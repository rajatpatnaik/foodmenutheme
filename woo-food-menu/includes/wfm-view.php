...
<form method="post" id="wfm_options">
  <?php
    //echo wfm_product_dropdown_categories( array(12), 1, 0, '' );
    echo wc_product_dropdown_categories( array(), 1, 0, '' );
    //die('okzzz');
  ?>  

  <input type="hidden" value="1" name="wfm_hval" />
  <input type="submit" class="wfm_search" name="wfm_btn_search" value="Search"/>
</form> <br /> 
  <?php
...
<script>
  ...
  //-------------------------------------
  var img_url_plus = '<? echo plugins_url(); ?>/woo-food-menu/images/plus.png';
  var img_url_minus = '<? echo plugins_url(); ?>/woo-food-menu/images/minus.png';
  ...
  function wfm_add_prod(pid,vid){
    ...
    ajax_url+='/woo-food-menu/includes/wfm-add-cart.php';    
        jQuery.ajax({
          type: "POST",
          url:ajax_url,
          data : {
  ...
  var plugin_url='<? echo plugins_url(); ?>/woo-food-menu/includes/wfm-popup-data.php';
</script>  
<?php
  ...
  if(isset($_POST['wfm_hval']) && isset($_POST['product_cat']) && $_POST['product_cat']!=''){
       ...
       wfm_show_prod2($loop, $wfm_img_size);
  }else{
    $exc_cats_slug=array();
...
function wfm_show_prod2($loop,$wfm_img_size){
   global $woocommerce;   
      if ($loop->have_posts()){        
        echo '<div class="submenu"><ul><table class="wfm_table">';
        foreach($loop->posts as $val){
          ...
          if($is_cat==0){
            ...
            if($variation==true && $product->is_type( 'variable' )){
              ...
              $product_name_org='<div class="wfm_name"><a href="'. plugins_url().'/woo-food-menu/includes/wfm-popup-data.php?pid='.$val->ID.'" class="simple-ajax-popup-align-top">'.$val->post_title.'</a></div>';
              $prod_des='';
              if($val->post_content){
                $prod_des=$val->post_content;
                if(strlen($prod_des)>=120){
                  $prod_des = substr($prod_des,0,140).'... <a href="'. plugins_url().'/woo-food-menu/includes/wfm-popup-data.php?pid='.$val->ID.'" class="simple-ajax-popup-align-top">Read More..</a>';
                }   
              }
              ...
              if (get_option('wfm_display_image_preview')=='1'){
                  echo '<td><a href="'.$img_url.'" class="preview"><img src="'.$img_url.'" height="'.$wfm_img_size.'" width="'.$wfm_img_size.'" /></a></td>';
                  }else{
                    echo '<td><img src="'.$img_url.'" height="'.$wfm_img_size.'" width="'.$wfm_img_size.'" /></td>';
              }
              if (get_option('wfm_display_image_preview')=='1'){
                echo '<td class="wfm_td">'.$product_name_org.'<div class="wfm_des">'.$prod_des.'</div></td><td>'.$prod_option.'</td>';
              }else{
                echo '<td class="wfm_td">'.$product_name_org.'<div class="wfm_des">'.$prod_des.'</div></td><td>'.$prod_option.'</td>';
              }  
...
function wfm_show_prod($id, $wfm_img_size, $post_title){
    ...
    $product_name='<div class="wfm_name"><a href="'. plugins_url().'/woo-food-menu/includes/wfm-popup-data.php?pid='.$id.'" class="simple-ajax-popup-align-top">'.$post_title.'</a>
      </div>';
    ...
    if($product->post->post_content){
      ...
      if(strlen($prod_des)>=120){
         $prod_des = substr($prod_des,0,120).'... <a href="'. plugins_url().'/woo-food-menu/includes/wfm-popup-data.php?pid='.$id.'" class="simple-ajax-popup-align-top">Read More..</a>';
      }
    }
    ...
    if (get_option('wfm_display_image_preview')=='1'){
      echo '<tr><td><a href="'.$img_url2.'" class="preview"><img src="'.$img_url.'" height="'.$wfm_img_size.'" width="'.$wfm_img_size.'" /></a></td>
        <td class="wfm_td">'.$product_name.'<div class="wfm_des">'.$prod_des.'</div></td>
          <td></td>';
    }else{                
      echo '<tr><td><img src="'.$img_url.'" height="'.$wfm_img_size.'" width="'.$wfm_img_size.'" /></td>
        <td class="wfm_td">'.$product_name.'<div class="wfm_des">'.$prod_des.'</div></td>
        ';
    }
    ?>
